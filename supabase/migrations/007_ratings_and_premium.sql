...
-- Note: company_user_id references the user ID (profile), not companies table
CREATE TABLE IF NOT EXISTS job_ratings (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  job_request_id UUID REFERENCES job_requests(id) ON DELETE CASCADE,
  technician_user_id UUID REFERENCES profiles(id) ON DELETE CASCADE,
  company_user_id UUID REFERENCES profiles(id) ON DELETE CASCADE,
  ...
  UNIQUE(job_request_id, company_user_id) -- One rating per job per company
);

-- Create premium_grants table if not exists
CREATE TABLE IF NOT EXISTS premium_grants (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  technician_id UUID REFERENCES profiles(id) ON DELETE CASCADE UNIQUE,
...
DROP POLICY IF EXISTS "Companies can insert ratings" ON job_ratings;
CREATE POLICY "Companies can insert ratings" ON job_ratings
  FOR INSERT WITH CHECK (company_user_id = auth.uid());

DROP POLICY IF EXISTS "Users can view relevant ratings" ON job_ratings;
CREATE POLICY "Users can view relevant ratings" ON job_ratings
  FOR SELECT USING (company_user_id = auth.uid() OR technician_user_id = auth.uid());

-- RLS policies for premium_grants
DROP POLICY IF EXISTS "Users can view own premium" ON premium_grants;
CREATE POLICY "Users can view own premium" ON premium_grants
