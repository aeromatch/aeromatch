...
export default function EngineAtaGridPage() {
  ...
  const book = engineId ? (getBook(engineId) as any) : null
  const isEngine = book?.engineSection === true
  const comingSoon = book?.comingSoon === true
  ...
  useEffect(() => {
    const run = async () => {
      if (!engineId) return
      setLoading(true)
      const qs = await fetchBookQuestions(engineId)
      setQuestions(qs)
      setLoading(false)
    }
    run()
        ...
        <div className="p-6 lg:p-8 max-w-6xl mx-auto">
          <div className="card p-8">
            <p className="text-error-400">
              {language === 'en' ? 'Engine not found.' : 'Motor no encontrado.'}
            </p>
            <Link
              href="/simulator/engines"
              className="text-gold-400 hover:text-gold-300 text-sm mt-4 inline-block"
            >
              ← {language === 'en' ? 'Back to engines' : 'Volver a motores'}
            </Link>
          </div>
        </div>
      </AppLayout>
  ...
  for (const q of questions) {
    if (!q?.ata) continue
    if (totalByAta.has(q.ata)) {
      totalByAta.set(q.ata, (totalByAta.get(q.ata) || 0) + 1)
    }
  }
      ...
      <div className="p-6 lg:p-8 max-w-6xl mx-auto">
        <div className="mb-6 flex items-center justify-between gap-4">
          ...
          <div className="text-right">
            <p className="text-xs text-steel-500">{language === 'en' ? 'Data' : 'Datos'}</p>
            <p className="text-sm text-white">
              {loading
                ? 'Loading…'
                : `${questions.length} ${language === 'en' ? 'questions' : 'preguntas'}`}
            </p>
          </div>
        </div>

        {comingSoon && questions.length === 0 && (
          <div className="card p-6 mb-6 border border-steel-700/30 bg-navy-900/40">
            <p className="text-white font-semibold">
              {language === 'en' ? 'Coming soon' : 'Próximamente'}
            </p>
            <p className="text-sm text-steel-400 mt-1">
              {language === 'en'
                ? 'Questions for this engine will be available in the next release.'
                : 'Las preguntas para este motor estarán disponibles en la próxima versión.'}
            </p>
          </div>
        )}
