...
/** Solo la «A» dorada del logotipo (hero, favores decorativos). No depende de PNG. */
export function LogoGoldA({
  className = '',
  size = 112,
}: {
  className?: string
  /** Ancho en px; alto proporcional al glifo */
  size?: number
}) {
  const h = Math.round(size * 1.22)
  return (
    <div className={`flex justify-center overflow-visible ${className}`} aria-hidden>
      <svg
        width={size}
        height={h}
        viewBox="-6 -14 102 128"
        preserveAspectRatio="xMidYMid meet"
        className="mx-auto block max-h-none overflow-visible"
        xmlns="http://www.w3.org/2000/svg"
      >
        <path
